#!/usr/bin/env python3
"""Plot Figure 5(a/b) from generated experiment CSVs."""
...
def draw_panel(
    ...
    width = 0.32
    maya = [float(row["maya_error_pct"]) for row in rows]
    flexeva = [float(row["flexeva_error_pct"]) for row in rows]
    ax.bar(
...
def draw(result_dir: Path, output_dir: Path) -> Path:
    ...
    draw_panel(ax_gpt, gpt_rows, "gpu_scale", "Number of GPUs", 5.0)
    ...
    output_path = output_dir / "figure5.pdf"
