...
try:
    ...
    def emit_step_marker(_kind: str, *, step: int | None = None, label: str = "training_step") -> None:
        return None

    @contextlib.contextmanager
    def step_window(_step: int | None = None):
        yield


_DIST_GROUP_TIMEOUT = timedelta(hours=2)
_VARIANT_MODULE = None
...
def _load_variant_module(path_text: str | None):
    global _VARIANT_MODULE
    if _VARIANT_MODULE is not None:
        return _VARIANT_MODULE
    if not path_text:
        return None
    path = Path(path_text).resolve()
    if not path.exists():
        raise FileNotFoundError(f"candidate variant entry does not exist: {path}")
    module_name = f"routed_moe_variant_{abs(hash(str(path)))}"
    spec = importlib.util.spec_from_file_location(module_name, path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load candidate variant entry: {path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = module
    spec.loader.exec_module(module)
    _VARIANT_MODULE = module
    return module


def _variant_module():
    return _load_variant_module(os.environ.get("ROUTED_MOE_VARIANT_ENTRY", "").strip() or None)


def configure_candidate_variant(args):
    module = _load_variant_module(args.variant_entry)
    if module is None:
        return args
    os.environ["ROUTED_MOE_VARIANT_ENTRY"] = str(Path(args.variant_entry).resolve())
    configure = getattr(module, "configure_args", None)
...
def apply_candidate_routing_variant(
    ...
    *,
    ...
    route_topk = getattr(module, "route_topk", None) if module is not None else None
    if route_topk is None:
    ...
    if isinstance(result, dict):
        new_indices = result.get("indices", top_k_indices)
        new_gates = result.get("gates", top_k_gates)
    else:
        new_indices, new_gates = result
    return (
        new_indices.to(device=gate_probs.device, dtype=torch.long),
        new_gates.to(device=gate_probs.device, dtype=gate_probs.dtype),
    )
...
def is_fakecuda_device(device: torch.device) -> bool:
    ...
    return any(
        os.environ.get(name)
        for name in ("FAKECUDA_TARGET_ENV_ROOT", "FAKECUDA_PROOT_BIN", "FAKECUDA_TRACE")
    )
...
def parse_forced_experts(value: str | None, *, top_k: int, num_experts: int) -> list[int] | None:
    if value is None:
        return None
    text = value.strip()
    if not text:
        return None
    experts = [int(part.strip()) for part in text.split(",") if part.strip()]
    if len(experts) != top_k:
        raise ValueError(f"expected {top_k} forced experts, got {experts}")
    if len(set(experts)) != len(experts):
        raise ValueError(f"forced experts must be unique, got {experts}")
    for expert in experts:
        if expert < 0 or expert >= num_experts:
            raise ValueError(f"forced expert {expert} outside [0, {num_experts})")
    return experts
...
def parse_args():
    ...
    ap.add_argument("--seed", type=int, default=42,
                    help="Base random seed. Different values generate different routing paths.")
    ap.add_argument("--route-path-id", type=int, default=-1,
                    help=(
                        "Force a deterministic routed-MoE expert path under fake-cuda. "
                        "Negative values keep the model-derived top-k routing."
                    ))
    ap.add_argument("--route-experts", default="",
                    help=(
                        "Comma-separated expert ids used as the forced top-k route. "
                        "Overrides --route-path-id when non-empty."
                    ))
    ap.add_argument("--route-p2p-probe", action="store_true",
                    help=(
                        "Issue sparse P2P sends/recvs to the forced expert-owner ranks. "
                        "This makes route endpoints visible in the trace."
                    ))
    ...
    ap.add_argument("--source-region-markers", action="store_true",
                    help="Emit source-level markers around attention/router backward and optimizer step")
    ap.add_argument("--variant-entry", default="",
                    help="Path to a source-level candidate variant module")
    ap.add_argument("--sync-before-step-window", action="store_true",
                    help="Synchronize all ranks through Gloo before each measured step")
    ap.add_argument("--timing-output", default="",
                    help="Optional rank-0 JSON path for measured training-step timings")
    ap.add_argument(
        "--section75-executor",
        choices=("maya-full", "flexeva-selective"),
        default="maya-full",
        help="Section 7.5 production gate: full step or affected MoE partitions only",
    )
    ap.add_argument(
        "--section75-dispatch-file",
        default="",
        help="Wait until this file exists after evaluator state initialization",
    )
    ap.add_argument(
        "--section75-feedback-output",
        default="",
        help="Rank-0 JSON output for the directly executed evaluator path",
    )
    ap.add_argument(
        "--section75-ready-file",
        default="",
        help="Rank-0 readiness record written before waiting for dispatch",
    )
...
def wait_for_section75_dispatch(path_text: str, ready_path_text: str, rank: int) -> None:
    if not path_text:
        return
    path = Path(path_text)
    dist.barrier()
    if rank == 0:
        ...
        print("FLEXEVA_SECTION75_WORKER_READY", flush=True)
        while not path.exists():
            time.sleep(0.01)
    dist.barrier()


def apply_runtime_candidate_config(model: nn.Module, args) -> None:
    for module in model.modules():
        if isinstance(module, TopKGate):
            module.capacity_factor = float(args.capacity_factor)
        elif isinstance(module, ExpertParallelMoELayer):
            module.expert_layout = str(args.expert_layout)
            module.expert_owner = module._build_expert_owner()
            module.local_global_experts = [
                expert_id
                for expert_id, owner in enumerate(module.expert_owner)
                if owner == module.rank_in_ep
            ]
        elif isinstance(module, MoETransformerBlock):
            module.recompute = bool(args.recompute)
...
class TopKGate(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        if fakecuda_cpu_routing:
            ...
            gate_probs_device = F.softmax(gate_logits, dim=-1)
            ...
            forced_experts = parse_forced_experts(
                os.environ.get("ROUTED_MOE_EXPERTS"),
                top_k=self.top_k,
                num_experts=self.num_experts,
            )
            forced_path = int(os.environ.get("ROUTED_MOE_PATH_ID", "-1"))
            if forced_experts is not None:
                selected = forced_experts
            elif forced_path >= 0:
                selected = [
                    (forced_path * max(self.top_k, 1) + offset) % self.num_experts
                    for offset in range(self.top_k)
                ]
            else:
                selected = []
            if selected:
                top_k_indices = torch.tensor(selected, dtype=torch.long).repeat(num_tokens, 1)
                top_k_gates = torch.full((num_tokens, self.top_k), 1.0 / max(self.top_k, 1))
            gate_probs = gate_probs_cpu.to(device=x.device, dtype=x.dtype)
            # FakeCUDA needs CPU values for data-dependent routing, while the
            # evaluator still needs the native softmax/router gradient path.
            gate_probs = gate_probs + gate_probs_device - gate_probs_device.detach()
            top_k_indices, top_k_gates = apply_candidate_routing_variant(
                gate_probs_cpu,
                top_k_indices,
                top_k_gates,
                x=x,
    ...
    def forward(self, x):
        enabled = bool(getattr(self, "_flexmaya_source_markers_enabled", False))
        with source_region("router_forward", enabled):
...
class TopKMoELayer(nn.Module):
    ...
    def forward(self, x):
        enabled = bool(getattr(self, "_flexmaya_source_markers_enabled", False))
        with source_region("expert_forward", enabled):
            return self._forward_impl(x)
...
class ExpertParallelMoELayer(nn.Module):
    ...
    def _forced_route_owner_ranks(self) -> list[int]:
        forced_experts = parse_forced_experts(
            os.environ.get("ROUTED_MOE_EXPERTS"),
        ...
        if forced_experts is None:
            ...
            if forced_path < 0:
                return []
            forced_experts = [
                (forced_path * max(self.gate.top_k, 1) + offset) % self.num_experts
                for offset in range(self.gate.top_k)
            ]
        return sorted({self.expert_owner[expert] for expert in forced_experts})

    def _route_p2p_probe(self, x_flat: torch.Tensor, owner_ranks: list[int]) -> None:
        if not owner_ranks or self.ep_group is None or self.ep_size <= 1:
            return
        enabled = os.environ.get("ROUTED_MOE_P2P_PROBE", "").strip().lower()
        if enabled not in {"1", "true", "yes", "on"}:
            return

        probe_tokens = max(1, min(16, int(x_flat.shape[0])))
        send_tensor = x_flat[:probe_tokens].contiguous()
        recv_buffers: list[torch.Tensor] = []
        ops = []
        rank = self.rank_in_ep
        for dst in owner_ranks:
            if dst != rank:
                ops.append(dist.P2POp(dist.isend, send_tensor, group=self.ep_group, group_peer=dst))
        if rank in owner_ranks:
            for src in range(self.ep_size):
                if src == rank:
                    continue
                recv_tensor = torch.empty_like(send_tensor)
                recv_buffers.append(recv_tensor)
                ops.append(dist.P2POp(dist.irecv, recv_tensor, group=self.ep_group, group_peer=src))
        if not ops:
            return
    ...
    def _forward_impl(self, x):
        ...
        if self.ep_group is not None and self.ep_size > 1:
            ...
            forced_owner_ranks = self._forced_route_owner_ranks()
            ...
            self._route_p2p_probe(x_flat, forced_owner_ranks)
...
class MoETransformerBlock(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        enabled = bool(getattr(self, "_flexmaya_source_markers_enabled", False))
        with source_region("attention_forward", enabled):
...
class SelectiveMoEStack(nn.Module):
    """The executable source partition used by Section 7.5 refresh."""
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            x, aux, metadata = layer(x)
            aux_total = aux if aux_total is None else aux_total + aux
        return x, aux_total, metadata
...
def attach_source_region_markers(model: nn.Module, enabled: bool) -> None:
    ...
    for module in model.modules():
        if isinstance(module, nn.MultiheadAttention):
            ...
            module._flexmaya_source_partition = "router_backward"
            module._flexmaya_source_markers_enabled = True
            module.register_full_backward_pre_hook(_source_backward_pre_hook)
            module.register_full_backward_hook(_source_backward_hook)
        elif isinstance(module, (TopKMoELayer, ExpertParallelMoELayer)):
            module._flexmaya_source_partition = "expert_backward"
            module._flexmaya_source_markers_enabled = True
            module.register_full_backward_pre_hook(_source_backward_pre_hook)
            module.register_full_backward_hook(_source_backward_hook)
        elif isinstance(module, MoETransformerBlock):
...
def run_section75_selective(
    ...
    *,
    ...
    wait_for_section75_dispatch(
        args.section75_dispatch_file,
        args.section75_ready_file,
        rank,
    )
    args = configure_candidate_variant(args)
    if args.recompute:
        raise RuntimeError(
            "candidate changes MoETransformerBlock recompute; the routing-partition selective executor must fail closed"
        )
    apply_runtime_candidate_config(model, args)

    batch_size = local_batch_size(args, dp_size)
    actual_micro_batches = min(int(args.micro_batches), batch_size)
    per_micro_batch = max(1, batch_size // max(actual_micro_batches, 1))
    metadata = None
    started = time.perf_counter()
    with step_window(1):
        for _ in range(actual_micro_batches):
            x = torch.randn(
                per_micro_batch,
                args.seq_len,
                args.hidden_size,
            ...
            with autocast_context(device, args.dtype):
                x, aux_total, metadata = model(x)
            (x.float().mean() + args.aux_weight * aux_total).backward()
    ...
    if rank == 0:
        payload = {
            "schema": "flexeva.section75.executor.v1",
            "executor": "flexeva-selective",
            "candidate_id": str(getattr(_variant_module(), "CANDIDATE_ID", "")),
            ...
            "tp": 1,
            "pp": 1,
            "micro_batches": int(actual_micro_batches),
            "num_layers": int(args.num_layers),
            "execute_wall_s": execute_wall_s,
            "routing": {
                "expert_counts": list(metadata.get("expert_counts", [])),
                "tokens_dropped": int(metadata.get("tokens_dropped", 0)),
                "capacity": int(metadata.get("capacity", 0)),
            },
        }
        if args.section75_feedback_output:
            output = Path(args.section75_feedback_output)
...
def main():
    args = parse_args()
    delayed_candidate = bool(args.section75_dispatch_file)
    if not delayed_candidate:
        args = configure_candidate_variant(args)
    route_experts = parse_forced_experts(
        args.route_experts,
        top_k=int(args.top_k),
        num_experts=int(args.num_experts),
    )
    if route_experts is not None:
        os.environ["ROUTED_MOE_EXPERTS"] = ",".join(str(expert) for expert in route_experts)
    if int(args.route_path_id) >= 0:
        os.environ["ROUTED_MOE_PATH_ID"] = str(int(args.route_path_id))
    if args.route_p2p_probe:
        os.environ["ROUTED_MOE_P2P_PROBE"] = "1"
    ...
    if args.section75_executor == "flexeva-selective":
        run_section75_selective(
            args,
            rank=rank,
            world_size=world_size,
            device=device,
    ...
    if rank == 0:
        print({
            ...
            "capacity_factor": args.capacity_factor,
            "micro_batches": args.micro_batches,
            "actual_micro_batches": actual_micro_batches,
            "recompute": args.recompute,
            "expert_layout": args.expert_layout,
            "seed": args.seed,
            "route_path_id": args.route_path_id,
            "route_experts": route_experts,
            "route_p2p_probe": args.route_p2p_probe,
    ...
    def run_single_step(step: int):
        ...
        for chunk_ids, chunk_labels in zip(micro_inputs, micro_labels):
            with autocast_context(device, args.dtype):
                with source_region("route_path", args.source_region_markers):
    ...
    measured_steps = []
    last_layer_metadata = None
    for step in range(1, args.steps + 1):
        if args.sync_before_step_window:
            dist.barrier(group=step_window_sync_group)
        step_started = time.perf_counter()
        with step_window(step):
            _, _, last_layer_metadata = run_single_step(step)
            if not args.no_step_end_synchronize:
                synchronize_completed_iteration(device)
        measured_steps.append(time.perf_counter() - step_started)

    dist.barrier()
    if rank == 0 and args.timing_output:
        ...
        timing_path.write_text(
            json.dumps(
                {
                    "executor": args.section75_executor,
                    "full_training_step_executed": True,
                    "full_candidate_trace_generated": True,
                    "fallback": False,
                    "variant_entry": args.variant_entry,
                    "steps": int(args.steps),
                    "warmup_steps": int(args.warmup_steps),
                    "measured_step_s": measured_steps,
                    "total_step_s": total_s,
                    "mean_step_s": total_s / max(len(measured_steps), 1),
                    "min_step_s": min(measured_steps) if measured_steps else 0.0,
                    "max_step_s": max(measured_steps) if measured_steps else 0.0,
                    "world_size": int(world_size),
                    "ep_size": int(ep_size),
                    "dp_size": int(dp_size),
                    "global_batch_size": args.global_batch_size,
                    ...
                    "micro_batches": int(args.micro_batches),
                    "actual_micro_batches": actual_micro_batches,
    ...
    if rank == 0 and args.section75_feedback_output:
        metadata = last_layer_metadata[0]
        payload = {
            "schema": "flexeva.section75.executor.v1",
            "executor": "maya-full",
            "candidate_id": str(getattr(_variant_module(), "CANDIDATE_ID", "")),
            ...
            "micro_batches": int(actual_micro_batches),
            "num_layers": int(args.num_layers),
            "execute_wall_s": sum(measured_steps),
            "routing": {
                "expert_counts": list(metadata.get("expert_counts", [])),
                "tokens_dropped": int(metadata.get("tokens_dropped", 0)),
                "capacity": int(metadata.get("capacity", 0)),
