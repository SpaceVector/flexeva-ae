...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run workload/routed-moe/moe_topk.py through fake-CUDA and compare Maya/FlexEva traces."
    )
    ...
    *,
    ...
    trace_dir = (
        ...
        if input_trace_root
            ...
            Path(output_trace_root) / case_dir.name / "traces"
            if output_trace_root
    ...
    torchrun_args, distributed = distributed_torchrun_args(config.world_size)
    command = [
        str(frun),
        str(args.python),
        "-m",
    ...
    transfer = transfer_remote_traces(trace_dir, config.world_size, distributed) if completed.returncode == 0 else None
    return {
        "command": command,
...
def main() -> int:
    ...
    result = {
        ...
        "method": {
            "capture": "workload/routed-moe/moe_topk.py through fake-CUDA frun",
