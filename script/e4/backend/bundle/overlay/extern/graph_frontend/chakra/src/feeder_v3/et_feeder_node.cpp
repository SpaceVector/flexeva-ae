...
std::vector<NodeId> ETFeederNode::data_deps() const {
  auto node = this->get_chakra_node();
  return std::vector<NodeId>(
      node->data_deps().begin(), node->data_deps().end());
}

std::vector<NodeId> ETFeederNode::ctrl_deps() const {
  auto node = this->get_chakra_node();
  return std::vector<NodeId>(
      node->ctrl_deps().begin(), node->ctrl_deps().end());
}
