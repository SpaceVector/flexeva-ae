...
def write_summary_files(args: argparse.Namespace, results: list[dict[str, object]]) -> None:
    ...
    readme = [
        ...
        f"- dtype: `{args.dtype}`",
        f"- timing: median of `{args.timing_repeats}` repetitions after `{args.timing_warmups}` warmups; IQR is retained in `summary.csv`.",
        "- paper mapping: submitted Table 6; the workload preset keeps its legacy `table4-lite` name.",
