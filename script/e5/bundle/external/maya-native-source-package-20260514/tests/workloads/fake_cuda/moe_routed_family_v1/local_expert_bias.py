#!/usr/bin/env python3
"""Sibling that biases routing toward current-rank experts when scores are close."""
...
class LocalExpertBiasTopKGate(BASE.TopKGate):
    """Add a small logit bias to experts owned by the current EP rank."""

    def __init__(self, hidden_size: int, num_experts: int, top_k: int = 2, capacity_factor: float = 1.25):
        super().__init__(hidden_size, num_experts, top_k, capacity_factor)
        self.local_bias = 0.20
    ...
    def forward(self, x):
        ...
        if dist.is_available() and dist.is_initialized() and dist.get_world_size() > 1:
            ...
            if self.num_experts % ep_size == 0:
                ...
                gate_logits[:, start:end] += self.local_bias
        ...
        if fakecuda_cpu_routing:
            ...
            if dist.is_available() and dist.is_initialized() and dist.get_world_size() > 1:
                ...
                if self.num_experts % ep_size == 0:
                    ...
                    gate_logits_cpu[:, start:end] += self.local_bias
...
if __name__ == "__main__":
    run_variant(
        FamilyVariant(
            variant_id="local_expert_bias",
            description="Code-level sibling: router logits slightly prefer current-rank experts when scores are close.",
            default_overrides={
                "batch_size": 2,
                "seq_len": 64,
                "hidden_size": 128,
            ...
            gate_cls=LocalExpertBiasTopKGate,
        )
    )
