...
class HookRecorder {
  ...
  private:
    ...
    std::unordered_set<int> capture_ranks_;
    bool has_capture_rank_filter_ = false;
    std::unordered_set<std::string> capture_code_partitions_;
    bool has_capture_code_partition_filter_ = false;
    std::string code_partition_policy_;
};
