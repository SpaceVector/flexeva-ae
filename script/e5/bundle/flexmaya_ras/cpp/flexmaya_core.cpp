...
std::string first_env_value(const char* primary, const char* fallback) {
    std::string value = env_value(primary);
    if (value.empty()) {
        value = env_value(fallback);
    }
    return value;
}
...
std::unordered_set<std::string> parse_string_set(const std::string& value) {
...
std::unordered_set<int> parse_rank_set(const std::string& value) {
    ...
    while (std::getline(stream, token, ',')) {
        ...
        selected.insert(std::stoi(token));
    }
    return selected;
}

bool contains_any_token(const std::string& value, std::initializer_list<const char*> tokens) {
...
std::string moe_v2_api_partition(const RawEvent& event) {
    const std::string api = lower_copy(event.api);
    const std::string kind = lower_copy(event.kind);
    if (contains_any_token(api, {"nccl", "alltoall", "all_to_all", "send", "recv"}) ||
        contains_any_token(kind, {"nccl", "collective"})) {
        return "dispatch_collective";
    }
    if (contains_any_token(api, {"gemm", "cublas", "matmul"}) || kind == "blas_compute") {
        return "expert_compute";
    }
    if (contains_any_token(api, {"memcpy", "malloc", "free"}) ||
        contains_any_token(kind, {"mem_copy", "mem_alloc", "mem_free"})) {
        return "memory_payload";
    }
    if (contains_any_token(api, {"synchronize", "event", "wait"}) || kind == "stream_op") {
        return "sync";
    }
...
std::vector<RawEvent> select_events(
    const std::vector<RawEvent>& raw_events,
    const std::vector<int>& selected_ranks,
    const std::vector<std::string>& selected_code_partitions) {
    const std::unordered_set<int> rank_filter(selected_ranks.begin(), selected_ranks.end());
    const std::unordered_set<std::string> code_filter(
        selected_code_partitions.begin(), selected_code_partitions.end());
    const bool filter_ranks = !rank_filter.empty();
    const bool filter_code_partitions = !code_filter.empty();
    std::vector<RawEvent> filtered;
    filtered.reserve(raw_events.size());
    for (const RawEvent& event : raw_events) {
        if (filter_ranks && rank_filter.count(event.rank) == 0) {
            continue;
        }
        if (filter_code_partitions && code_filter.count(event.code_partition) == 0) {
            continue;
        }
        filtered.push_back(event);
    }
...
HookRecorder::HookRecorder() : local_arena_(std::make_shared<EventArena>()) {
    std::string shm_name = first_env_value("FLEXMAYA_SHM_NAME", "PLAIN_MAYA_SHM_NAME");
    default_code_partition_ = first_env_value("FLEXMAYA_CODE_PARTITION", "PLAIN_MAYA_CODE_PARTITION");
    code_partition_policy_ = lower_copy(first_env_value(
        "FLEXMAYA_CODE_PARTITION_POLICY",
        "PLAIN_MAYA_CODE_PARTITION_POLICY"));
    const std::string capture_ranks = first_env_value("FLEXMAYA_CAPTURE_RANKS", "PLAIN_MAYA_CAPTURE_RANKS");
    if (!capture_ranks.empty()) {
        capture_ranks_ = parse_rank_set(capture_ranks);
        has_capture_rank_filter_ = true;
    }
    const std::string capture_partitions = first_env_value(
        "FLEXMAYA_CAPTURE_CODE_PARTITIONS",
        "PLAIN_MAYA_CAPTURE_CODE_PARTITIONS");
    if (!capture_partitions.empty()) {
        capture_code_partitions_ = parse_string_set(capture_partitions);
        has_capture_code_partition_filter_ = true;
    }
    if (!shm_name.empty()) {
        try {
...
std::uint64_t HookRecorder::record(const RawEvent& event) {
    RawEvent row = event;
    if (row.code_partition.empty() && code_partition_policy_ == "moe_v2_api") {
        row.code_partition = moe_v2_api_partition(row);
    }
    if (row.code_partition.empty() && !default_code_partition_.empty()) {
        row.code_partition = default_code_partition_;
    }
    if (has_capture_rank_filter_ && capture_ranks_.count(row.rank) == 0) {
        return 0;
    }
    if (has_capture_code_partition_filter_ && capture_code_partitions_.count(row.code_partition) == 0) {
        return 0;
    }
    if (shared_arena_) {
...
GlobalTrace build_rank_grouped_selected_trace_ras(
    const std::vector<RawEvent>& raw_events,
    const std::map<int, std::vector<int>>& rank_groups,
    const std::vector<int>& selected_ranks,
    const std::vector<std::string>& selected_code_partitions) {
    const std::vector<RawEvent> selected = select_events(raw_events, selected_ranks, selected_code_partitions);
    return build_rank_grouped_trace_ras(selected, rank_groups);
}
