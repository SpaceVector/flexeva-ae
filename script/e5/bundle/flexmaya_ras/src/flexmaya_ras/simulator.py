...
def _duration_us(event: object, predictor: ReplayRandomForestPredictor) -> float:
    kind = str(getattr(event, "kind", ""))
    hinted = float(getattr(event, "duration_hint_us", 0.0) or 0.0)
    if hinted > 0.0:
        return hinted
    ...
    *,
    ...
    while ready:
        ...
        if partition is not None:
            ...
            duration = max(_duration_us(event_by_id[member], predictor) for member in members)
        ...
        finish = start + _duration_us(event, predictor)
