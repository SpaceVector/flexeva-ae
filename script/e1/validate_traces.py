#!/usr/bin/env python3
"""Validate the linked 128-GPU E1 traces against the Figure 1 ledger."""
...
DEFAULT_LEDGER = ROOT / "result" / "e1" / "trajectory.csv"
DEFAULT_OUTPUT = ROOT / "result" / "e1" / "trace_validation.csv"
...
def main() -> int:
    ...
    rows = validate(args.trace_root, args.ledger, args.a2a_normalized_tolerance)
    if not args.check_only:
        write_rows(args.output, rows)
    max_delta = max(float(row["a2a_normalized_delta"]) for row in rows)
    print(f"E1 linked traces: PASS (5 rounds, 128 ranks, max A2A delta {max_delta:.6f})")
    return 0
