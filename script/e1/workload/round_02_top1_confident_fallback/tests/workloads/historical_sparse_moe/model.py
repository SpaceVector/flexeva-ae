...
class HistoricalTop2PackedMoE(nn.Module):
    """Top-2 routed MoE with packed variable-length dispatch and confident-primary fallback."""
    ...
    def _route_tokens(self, flat: torch.Tensor) -> tuple[torch.Tensor, MoEForwardStats]:
        ...
        for token_idx in range(num_tokens):
            primary_score = float(top_scores[token_idx, 0].item())
            secondary_score = float(top_scores[token_idx, 1].item())
            score_gap = primary_score - secondary_score
            confident_primary = (primary_score > 0.80) or (score_gap > 0.20)
            ...
            for choice_idx in range(self.model_cfg.top_k):
                ...
                if current_load >= capacity:
                    dropped += 1
                    continue
                if choice_idx > 0 and confident_primary:
                    continue
                assigned_tokens.append(token_idx)
                assigned_experts.append(expert)
                assigned_scores.append(float(top_scores[token_idx, choice_idx].item()))
...
class TransformerBlock(nn.Module):
    def __init__(self, cfg: HistoricalSparseMoEWorkloadConfig, ep_group: dist.ProcessGroup | None, use_moe: bool):
        ...
        self.ffn = HistoricalTop2PackedMoE(cfg, ep_group) if use_moe else DenseFFN(cfg.model)
