...
This directory contains small variants of [`moe_topk.py`](../moe_topk.py) used
to test routing, layout, and dispatch changes. `anchor_baseline.py` is the
baseline; each remaining entry changes one policy or code path.
...
Shared code is in [`common.py`](common.py).
