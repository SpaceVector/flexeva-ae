...
def source_hashes(spec: FlexMayaWorkloadSpec) -> tuple[SourceHash, ...]:
    ...
    for partition in spec.code_partitions:
        if partition.partition_id in seen_partition_ids:
            raise ValueError(f"duplicate code partition id: {partition.partition_id}")
        seen_partition_ids.add(partition.partition_id)
...
def _changed_partitions(
    anchor: FlexMayaAnchor,
    candidate: FlexMayaWorkloadSpec,
    *,
    candidate_source_hashes: tuple[SourceHash, ...] | None = None,
) -> tuple[str, ...]:
    after_hashes = candidate_source_hashes if candidate_source_hashes is not None else source_hashes(candidate)
    before_partitions = {item.partition_id: item for item in anchor.source_hashes if item.partition_id is not None}
    after_partitions = {item.partition_id: item for item in after_hashes if item.partition_id is not None}
    changed: list[str] = []
    for partition in candidate.code_partitions:
        before = before_partitions.get(partition.partition_id)
        after = after_partitions[partition.partition_id]
    ...
    before_files = {
        (item.path, item.start_line, item.end_line): item.sha1
        for item in anchor.source_hashes
        if item.partition_id is None
    }
    after_files = {
        (item.path, item.start_line, item.end_line): item.sha1
        for item in after_hashes
        if item.partition_id is None
    }
    if not changed and before_files != after_files:
...
def plan_candidate_refresh(
    ...
    *,
    ...
    source_changed_partitions = _changed_partitions(
        anchor,
        candidate_spec,
        candidate_source_hashes=candidate_source_hashes,
    )
    changed_partitions = source_changed_partitions
    ...
    source_affected_trace_partitions = _affected_trace_partitions(trace, source_changed_partitions)
    if config_changed:
        config_affected_trace_partitions, config_reasons = _config_affected_trace_partitions(anchor.spec, candidate_spec, trace)
        fallback_reasons.extend(config_reasons)
        if source_affected_trace_partitions:
            fallback_reasons.append(
                "source changes also affect "
                + str(len(source_affected_trace_partitions))
                + " trace windows; refreshing the union with config-dependent windows"
            )
        affected_trace_partitions = tuple(
            sorted(set(source_affected_trace_partitions).union(config_affected_trace_partitions))
        )
    else:
        affected_trace_partitions = source_affected_trace_partitions
    affected_rank_groups = _affected_groups_from_events(
        trace,
        _event_ids_in_trace_partitions(trace, affected_trace_partitions),
    )
    if (
        not grounding_satisfied
        and any(partition.requires_grounding for partition in candidate_spec.code_partitions)
    ):
